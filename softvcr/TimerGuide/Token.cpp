...
{
	if (( m_strLeft.FindOneOf(m_strToken) == -1) && (m_bFinalToken))
	{
		return TRUE; // The string that is left over is a token
	}
	else if (( m_strLeft.FindOneOf(m_strToken) == -1) && (!m_bFinalToken))
	{
...
CString CToken::GetNextToken()
{
	...
	pos = newtok.FindOneOf(m_strToken);
	if (pos == -1)
	{
	...
	if ( m_strLeft.FindOneOf(m_strToken) == -1)
	{
